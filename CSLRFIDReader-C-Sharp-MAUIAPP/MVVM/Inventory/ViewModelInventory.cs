...
using CSLHandheldReader_C_Sharp_MAUIAPP.Models;
using CSLibrary;
using CSLibrary.Structures;

namespace CSLHandheldReader_C_Sharp_MAUIAPP.Maui.ViewModels
{
    public class ViewModelInventory : INotifyPropertyChanged
    {
        ...
        public string EPCHeaderText { get; private set; }
        public string RSSIHeaderText { get; private set; }
        public ObservableCollection<TagInfo> TagsInfo { get; private set; }
        ...
        public ViewModelInventory()
        {
            ...
            TagsInfo = new ObservableCollection<TagInfo>();
            EPCHeaderText = "EPC";
            RSSIHeaderText = "RSSI";
        }
        ...
        private void SetEvent(bool enable)
        {
            ...
            GlobalVariable._reader.barcode.ClearEventHandler();
            ...
            {
                // RFID event handler
                GlobalVariable._reader.rfid.OnAsyncCallback += new EventHandler<CSLibrary.Events.OnAsyncCallbackEventArgs>(TagInventoryEvent);
                //obalVariable._reader.rfid.OnStateChanged += new EventHandler<CSLibrary.Events.OnStateChangedEventArgs>(StateChangedEvent);
                //GlobalVariable._reader.rfid.OnInventoryTagRateCallback += new EventHandler<CSLibrary.Events.OnInventoryTagRateCallbackEventArgs>(InventoryTagRateCallback);

                // Barcode event handler
                //GlobalVariable._reader.barcode.OnCapturedNotify += new EventHandler<CSLibrary.Barcode.BarcodeEventArgs>(Linkage_CaptureCompleted);
        ...
        void InventorySetting()
        {
            ...
            GlobalVariable._reader.rfid.Options.TagRanging.flags = CSLibrary.Constants.SelectFlags.ZERO;

            GlobalVariable._reader.rfid.SetInventoryDuration(0);
            GlobalVariable._reader.rfid.SetTagDelayTime(0); // for CS108 only
            GlobalVariable._reader.rfid.SetIntraPacketDelayTime(0); // for CS710S only
            GlobalVariable._reader.rfid.SetDuplicateEliminationRollingWindow(0); // for CS710S only
            GlobalVariable._reader.rfid.SetCurrentLinkProfile(244);
            GlobalVariable._reader.rfid.SetTagGroup( CSLibrary.Constants.Selected.ALL, CSLibrary.Constants.Session.S0, CSLibrary.Constants.SessionTarget.A);
            //            GlobalVariable._reader.rfid.SetHoppingChannels();

            DynamicQParms dynParm = new DynamicQParms();

            dynParm.toggleTarget =  1U;
            GlobalVariable._reader.rfid.SetDynamicQParms(dynParm);

            /*
            if (GlobalVariable._config.RFID_Algorithm == CSLibrary.Constants.SingulationAlgorithm.DYNAMICQ)
            {
                GlobalVariable._config.RFID_DynamicQParms.toggleTarget = GlobalVariable._config.RFID_ToggleTarget ? 1U : 0;
                GlobalVariable._reader.rfid.SetDynamicQParms(GlobalVariable._config.RFID_DynamicQParms);
            }
            else
            {
                GlobalVariable._config.RFID_FixedQParms.toggleTarget = GlobalVariable._config.RFID_ToggleTarget ? 1U : 0;
                GlobalVariable._reader.rfid.SetFixedQParms(GlobalVariable._config.RFID_FixedQParms);
            }

            // Select Criteria filter
            if (GlobalVariable._PREFILTER_Enable)
            {
                ...
                if (GlobalVariable._PREFILTER_Bank == 1)
                {
                    GlobalVariable._reader.rfid.Options.TagSelected.bank = CSLibrary.Constants.MemoryBank.EPC;
                    GlobalVariable._reader.rfid.Options.TagSelected.epcMask = new CSLibrary.Structures.S_MASK(GlobalVariable._PREFILTER_MASK_EPC);
                    GlobalVariable._reader.rfid.Options.TagSelected.epcMaskOffset = GlobalVariable._PREFILTER_MASK_Offset;
                    GlobalVariable._reader.rfid.Options.TagSelected.epcMaskLength = (uint)(GlobalVariable._PREFILTER_MASK_EPC.Length) * 4;
                }
                else
                {
                    GlobalVariable._reader.rfid.Options.TagSelected.bank = (CSLibrary.Constants.MemoryBank)(GlobalVariable._PREFILTER_Bank);
                    GlobalVariable._reader.rfid.Options.TagSelected.Mask = CSLibrary.Tools.Hex.ToBytes(GlobalVariable._PREFILTER_MASK_EPC);
                    GlobalVariable._reader.rfid.Options.TagSelected.MaskOffset = GlobalVariable._PREFILTER_MASK_Offset;
                    GlobalVariable._reader.rfid.Options.TagSelected.MaskLength = (uint)(GlobalVariable._PREFILTER_MASK_EPC.Length) * 4;
                }
                GlobalVariable._reader.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_PREFILTER);
                GlobalVariable._reader.rfid.Options.TagRanging.flags |= CSLibrary.Constants.SelectFlags.SELECT;
            }

            GlobalVariable._reader.rfid.SetRSSIdBmFilter(GlobalVariable._RSSIFILTER_Type, GlobalVariable._RSSIFILTER_Option, GlobalVariable._RSSIFILTER_Threshold_dBm);
            */
            ...
            GlobalVariable._reader.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_PRERANGING);
            ...
            //SetConfigPower();
            GlobalVariable._reader.rfid.SetPowerLevel(300);
        }

        void SetConfigPower()
        {
/*
            if (GlobalVariable._reader.rfid.GetAntennaPort() == 1)
            {
                if (GlobalVariable._config.RFID_PowerSequencing_NumberofPower == 0)
                {
                    GlobalVariable._reader.rfid.SetPowerSequencing(0);
                    GlobalVariable._reader.rfid.SetPowerLevel(GlobalVariable._config.RFID_Antenna_Power[0]);
                }
                else
                    GlobalVariable._reader.rfid.SetPowerSequencing(GlobalVariable._config.RFID_PowerSequencing_NumberofPower, GlobalVariable._config.RFID_PowerSequencing_Level, GlobalVariable._config.RFID_PowerSequencing_DWell);
            }
            else
            {
                uint port = GlobalVariable._reader.rfid.GetAntennaPort();

                for (uint cnt = 0; cnt < port; cnt++)
                {
                    GlobalVariable._reader.rfid.SetPowerLevel(GlobalVariable._config.RFID_Antenna_Power[cnt], cnt);
                }
            }
*/
        }
        ...
        void StartInventory()
        {
            ...
            //_ListViewRowHeight = 40 + (int)(GlobalVariable._reader.rfid.Options.TagRanging.multibanks * 10);
            //OnPropertyChanged(nameof(ListViewRowHeight));

            InventoryStartTime = DateTime.Now;

            _Vibrating = false;
            _noNewTag = 0;
            if (GlobalVariable._config.RFID_Vibration)
                GlobalVariable._reader.barcode.VibratorOn(CSLibrary.BarcodeReader.VIBRATORMODE.INVENTORYON, GlobalVariable._config.RFID_VibrationTime);

            GlobalVariable._reader.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_RANGING);
            CSLHandheldReader_C_Sharp_MAUIAPP.Converters.ClassBattery.SetBatteryMode(CSLHandheldReader_C_Sharp_MAUIAPP.Converters.ClassBattery.BATTERYMODE.INVENTORY);
            _cancelVoltageValue = true;

            OnPropertyChanged(nameof(startInventoryButtonText));
        }
...
*/        }
        ...
        void TagInventoryEvent(object sender, CSLibrary.Events.OnAsyncCallbackEventArgs e)
        {
            ...
            MainThread.BeginInvokeOnMainThread(async () =>
            {
                ...
                {
                    if (GlobalVariable._config.RFID_InventoryAlertSound)
                    {
                        MainThread.BeginInvokeOnMainThread(async () =>
                        {
        ...
        private void AddOrUpdateTagData(CSLibrary.Structures.TagCallbackInfo info)
        {
            ...
            MainThread.BeginInvokeOnMainThread(() =>
            {
                lock (TagsInfo)
                {
                    try
                    {
                        TagInfoListSpeedup.Add(epcstr, (TagsInfo.Count, null));

                        TagInfo item = new TagInfo();

                        item.timeOfRead = DateTime.Now;
                        item.EPC = info.epc.ToString();
                        item.EPC_Display = item.EPC;
                        ...
                        TagsInfo.Insert(0, item);
                    }
                    catch (Exception ex)
                    {
                        if (TagInfoListSpeedup.TryGetValue(epcstr, out var values))
                        {
                            var existingItem = TagsInfo[values.index];
                            existingItem.Bank1Data = CSLibrary.Tools.Hex.ToString(info.Bank1Data);
                            existingItem.Bank2Data = CSLibrary.Tools.Hex.ToString(info.Bank2Data);
                            existingItem.RSSI = info.rssidBm;
                            existingItem.timeOfRead = DateTime.Now;
                        }
                    }
                }
            });
...
namespace CS710S_Inventory_MQTT_broker.Maui.ViewModels
{
